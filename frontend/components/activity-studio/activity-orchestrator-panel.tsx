...
import { streamActivityOrchestrator } from "@/lib/stream";
import type { ActivityAssistantStep, ActivityTypeDraft, StreamEvent } from "@/lib/types/api";
...
type ActivityOrchestratorPanelProps = {
  locale: Locale;
...
export function ActivityOrchestratorPanel({
  locale,
  step,
  draft,
  disabled = false,
}: ActivityOrchestratorPanelProps) {
  ...
  const copy = useMemo(
    () =>
      locale === "he"
        ? {
            title: "מנחה יצירת סוג פעילות",
            subtitle: "עזרה בהחלטות ושלבי האשף.",
            placeholder: "שאל על מיקום, מודול, אוטומציה…",
            send: "שליחה",
            sending: "שולח…",
            empty: "שאל את המנחה לגבי השלב הנוכחי.",
            statusPreparing: "מכין תשובה…",
          }
        : {
            title: "Activity setup guide",
            subtitle: "Decision help for the current wizard step.",
            placeholder: "Ask about placement, module, automation…",
            send: "Send",
            sending: "Sending…",
            empty: "Ask the guide about the current step.",
  ...
  async function sendMessage() {
    ...
    try {
      await streamActivityOrchestrator(
        {
          step,
          draft,
